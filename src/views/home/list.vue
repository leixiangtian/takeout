<template>
  <div>
    <van-list
      v-model="loading"
      :finished="finished"
      finished-text="没有更多了..."
      @load="getData"
    >
      <ul class="list-container">
        <li
          class="shop-list"
          v-for="item in list"
          @click="goDetail(item.id)"
          :key="item.id"
        >
          <div class="img-box">
            <img :src="item.img" alt="" />
          </div>
          <div class="info-box">
            <div class="title">{{ item.name }}</div>
            <div class="rate-box">
              <div>
                <star :score="item.score"></star>
                <span class="rate">{{ item.score }}</span>
                <span>月销{{ item.num }}</span>
              </div>
              <div>
                <span>{{ item.minute }}分钟</span>
                <span class="left-line">距离{{ item.distance }}</span>
              </div>
            </div>
            <div class="distriibution-box">
              <span>起送{{ item.per_capita }}</span>
              <span class="left-line">配送{{ item.fee }}</span>
              <span class="left-line">人均{{ item.price }}</span>
            </div>
          </div>
        </li>
      </ul>
    </van-list>
  </div>
</template>

<script>
import star from '@/components/star/index.vue' //五角星
import { getList } from '@/api/home.js' //导入home页通用数据接口文件的getList

import axios from 'axios'

export default {
  data() {
    return {
      current: 0, //当前页数
      size: 10, //显示的条数
      list: [], //数据储存
      loading: false, //加载状态
      finished: false, //是否加载完成
      total: 0, //总数
    }
  },
  components: {
    star,
  },
  methods: {
    //获取商家列表
    getData() {
      var that = this
      // setTimeout(function() {
      //   Request(url, {
      //     current: that.current,
      //     size: that.size
      //   }).then(res => {
      //     //that.list = res.data.list;
      //     // console.log(that.list);
      //     that.list = that.list.concat(res.data.list);
      //     that.loading = false;
      //     that.current++;
      //     that.total = res.data.total;
      //     if (that.list.length >= that.total) {
      //       that.finished = true;
      //     }
      //   })
      // }, 500)
      setTimeout(() => {
        getList({
          current: that.current,
          size: that.size,
        }).then(function(res) {
          console.log(res)
          that.list = that.list.concat(res.data.list)
          that.loading = false
          that.current++
          that.total = res.data.total
          if (that.list.length >= that.total) {
            that.finished = true
          }
        })
      }, 500)
    },
    goDetail(id) {
      this.$router.push({
        path: '/detail',
        query: {
          id,
        },
      })
      //console.log(id);
    },
  },
  mounted() {
    axios
      .get('http://api.lxtian.cn/take-out/get_store/')
      .then((res) => {
        console.log(res.data)
      })
      .catch((err) => {
        alert('请求失败')
      })
  },
}
</script>

<style lang="scss" scoped>
.list-container {
  padding: 0 0.2rem;

  .shop-list {
    margin: 0.2rem 0 0.5rem 0;
    display: flex;

    .img-box {
      width: 1.52rem;
      height: 1.14rem;
      margin-right: 0.16rem;

      img {
        width: 100%;
        height: 100%;
      }
    }

    .info-box {
      flex: 1;

      .title {
        font-size: 0.32rem;
        color: #333;
        line-height: 1.4;
        font-weight: 600;
      }

      .rate-box,
      .distriibution-box {
        display: flex;
        color: #333;
        line-height: 1.4;
        font-family: mtsi-font;
        font-size: 11px;

        .rate {
          margin-right: 0.2rem;
        }

        .left-line::before {
          content: '';
          display: inline-block;
          width: 1px;
          height: 6px;
          background: #ccc;
          margin: 0 3px;
        }
      }

      .rate-box {
        justify-content: space-between;
      }
    }
  }
}
</style>
